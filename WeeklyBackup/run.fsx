...
let Run (sunday0220:TimerInfo, log:TraceWriter) =
  log.Info( sprintf "WeeklyBackup function executed at: %s" (DateTime.Now.ToString()) )
  ...
  let weeklyBackupContainer = CloudStorageAccount.Parse(storageConnString).CreateCloudBlobClient().GetContainerReference("weekly-backup")
  ...
  weeklyBackupContainer.GetBlockBlobReference( latestBackup.Name ).StartCopy( latestBackup ) |> ignore

  // Delete old backups
  let backups =
    weeklyBackupContainer.ListBlobs()
    |> Seq.toList
    |> List.map (fun (b:IListBlobItem) -> b :?> CloudBlockBlob)
